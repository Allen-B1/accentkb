...
function insertString(str) {
  ...
  if (elem instanceof HTMLInputElement || elem instanceof HTMLTextAreaElement) {
    var start = typeof elem.selectionStart == "number" ? elem.selectionStart : elem.value.length;
    var end = typeof elem.selectionEnd == "number" ? elem.selectionEnd : start;
    
    elem.value = elem.value.slice(0, start) + str + elem.value.slice(end);

    elem.selectionStart = elem.selectionEnd = start + str.length;
  }
}
